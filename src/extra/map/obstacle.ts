...
const ObstaclePointColor = '#ff4747';
const ObstacleLineColor = '#ff4747';
const ObstacletPointSize = 5;

export class Obstacle extends Polygon {
  ...
  public draw(context: CanvasRenderingContext2D): void {
    ...
    context.fillStyle = ObstaclePointColor;
    context.strokeStyle = ObstacleLineColor;
    for (let i = 0; i < drawingPoints.length; i++) {
      const x = drawingPoints[i].getPosition().x;
      const y = drawingPoints[i].getPosition().y;
      context.moveTo(x, y);
      context.arc(x, y, ObstacletPointSize, 0, 2 * Math.PI);
      context.fill();
      if (i > 0) {
        context.beginPath();
